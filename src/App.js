...
export default function App(){
  ...
  const [history, setHistory] = useState(()=>{
    try{ return JSON.parse(localStorage.getItem('prompts')||'[]') }catch(e){return []}
  });

  useEffect(()=>{ localStorage.setItem('prompts', JSON.stringify(history)) }, [history]);
        ...
        <div style={{display:'grid',gridTemplateColumns:'1fr 260px',gap:12,marginTop:12}}>
          <div>
            ...
            <div className="actions">
              ...
              <button className="btn" style={{background:'transparent',border:'1px solid rgba(255,255,255,0.06)'}} onClick={()=>{
                setResult('');
              }}>Clear</button>
            </div>
            {result && (
          ...
          <aside className="sidebar">
            ...
            <div className="panel" style={{padding:14,marginTop:12}}>
              ...
              <div style={{marginTop:8,display:'flex',flexDirection:'column',gap:8}}>
                {history.map(h=> (
                  <div key={h.id} style={{padding:10,borderRadius:8,background:'rgba(255,255,255,0.02)',cursor:'pointer'}} onClick={()=>setResult(h.prompt)}>
              ...
              <div style={{marginTop:10,display:'flex',gap:8}}>
                ...
                <button className="btn" style={{background:'transparent',border:'1px solid rgba(255,255,255,0.06)'}} onClick={()=>{ setHistory([]); }}>Clear</button>
              </div>
            </div>
          </aside>
        </div>
